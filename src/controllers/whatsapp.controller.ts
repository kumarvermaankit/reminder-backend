...
export class WhatsappController {
  ...
  private estimateCalories(description: string): number {
    ...
    const foods: [RegExp, number][] = [
      [/chicken (breast|sandwich|salad|biryani|curry|tikka)/, 350],
      [/rice|biryani|pulao/, 250],
      [/roti|chapati|phulka|naan/, 120],
      [/dal|daal|lentil/, 150],
      [/sandwich|burger/, 400],
      [/pizza/, 600],
      [/pasta|maggi|noodles|macroni/, 400],
      [/egg|omelette/, 150],
      [/milk|doodh/, 120],
      [/curd|yogurt|dahi/, 100],
      [/paneer/, 250],
      [/samosa/, 200],
      [/dosa|idli/, 150],
      [/paratha/, 250],
      [/sabzi|vegetable|bhaji/, 100],
      [/salad/, 80],
      [/fruit|apple|banana|orange|mango/, 100],
      [/tea|chai|coffee/, 50],
      [/juice|smoothie/, 150],
      [/cake|cookie|biscuit|pastry|donut/, 300],
      [/ice.?cream|kulfi/, 250],
      [/chocolate|candy|chips/, 200],
      [/fish|prawn|shrimp|salmon|tuna/, 300],
      [/mutton|lamb|beef|pork/, 350],
      [/soup/, 120],
    ];
    for (const [regex, cal] of foods) {
      if (regex.test(lower)) return cal;
    }
    // Portion-based: if number mentioned, use it
    const portionMatch = lower.match(/(\d+)\s*(g|gram|ml)/);
    if (portionMatch) {
      const grams = parseInt(portionMatch[1], 10);
      return Math.round(grams * 1.5); // ~1.5 kcal per gram average
    }
    return 250; // default
  }
